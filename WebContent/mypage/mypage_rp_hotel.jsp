...
ArrayList<ReplyInfo> hotel_rp_list = (ArrayList<ReplyInfo>) request.getAttribute("hotel_rp_list"); // 호텔 후기 리스트
%>
...
.profile {
	...
	<div class="contents" style="padding:0px;">
		<h4 style="margin-top:25px;margin-left:10px;">내가 작성한 숙소 후기</h4>
		<div class="my-3 p-3 bg-body rounded shadow-sm" style="display: flex;flex-wrap: wrap;"> 
	
			<%
			if (hotel_rp_list != null && hotel_rp_list.size() > 0) {
				for (int i = 0; i < hotel_rp_list.size(); i++) {
					ReplyInfo info = hotel_rp_list.get(i);
			%>
			<div class="card" style="margin-right: 10px;width: 48%;margin:10px;padding:10px;">
				<div class="card-body">
					<p><a href="hotel_view.hotel?hotel_code=<%=info.getHotel_code() %>"><%=info.getHotel_name() %> | <%= common.getRoomType().get(info.getRoom_type()) %></a></p>
