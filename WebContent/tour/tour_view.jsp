...
<div class="contents">
	...
	<div class="container" style="margin-top:30px;margin-bottom:50px;">
		<div class="row featurette">
			<h2>[<%=common.getTourType().get(info.getTour_type()) %>] <%=info.getTour_name() %> | <%=common.getArea().get(info.getTour_area()) %></h2>
			...
			<div id="myCarousel" class="carousel slide" data-bs-ride="carousel" style="margin-bottom: 1rem;">
				<div class="carousel-indicators">
					...
					<% if ( info.getTour_img2()!=null && !info.getTour_img2().equals("")) { %>
					<button type="button" data-bs-target="#myCarousel" data-bs-slide-to="1" aria-label="Slide 2" class=""></button>
					<% } %>
					<% if ( info.getTour_img3()!=null && !info.getTour_img3().equals("")) { %>
					...
					<div class="carousel-item active">
						<svg class="bd-placeholder-img" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" preserveAspectRatio="xMidYMid slice" focusable="false">
							<rect width="100%" height="100%" fill="#777">
								<img src="<%=tourImgSrc+info.getTour_img1() %>" />
							</rect> 
						</svg>			 
					</div>
				<% if ( info.getTour_img2()!=null && !info.getTour_img2().equals("")) { %>
					<div class="carousel-item">
						<svg class="bd-placeholder-img" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" preserveAspectRatio="xMidYMid slice" focusable="false">
							<rect width="100%" height="100%" fill="#777">
								<img src="<%=tourImgSrc+info.getTour_img2() %>" />
							</rect>
						</svg> 
					</div> 
				<% } %>
				<% if ( info.getTour_img3()!=null && !info.getTour_img3().equals("")) { %>
					<div class="carousel-item">
						<svg class="bd-placeholder-img" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" preserveAspectRatio="xMidYMid slice" focusable="false">
							<rect width="100%" height="100%" fill="#777">
								<img src="<%=tourImgSrc+info.getTour_img3() %>" />
							</rect>
						</svg> 
					</div> 
				<% } %>
	      	...
	      	<div class="col-md-12 order-md-2">
	        	<h4><span class="text-muted"><%=info.getTour_addr() %></span></h4>
	        	<p class="lead" style="margin-top:10px;"><%=common.getStar().get((double)info.getTour_score())%>  </p>
	        	<p class="lead" style="margin-top:10px;"><strong>평균 <%=info.getTour_score()%>점 /</strong> <%=info.getTour_review_cnt() %>개의 후기</p>
	        	<p class="lead" style="margin-top:10px;">#<%=info.getTour_tag().replace(",", " #")%></p>
	        	<p class="lead" style="margin-top:10px;"><%=info.getTour_detail() %></p> 
	      	</div>
	    </div>
	</div>
	...
	<div class="row featurette">
		<div class="mb-12" >
			...
			<span style="float:right;"><a href="tour_reply_form.tour?tour_code=<%=info.getTour_code()%>"><span class="btn btn-outline-dark">후기작성</span></a></span>
		</div> 
	...
	<div class="p-3 bg-body rounded shadow-sm">
		...
		<% } %>
			...
			<span>
				...
				<a href="tour_reply_del.tour?tour_code=<%=replyInfo.getReply_code()%>&reply_idx=<%=replyInfo.getReply_idx()%>&mi_id=<%=mi_id%>">x</a>
				<% } %>
			</span>
		</div>
