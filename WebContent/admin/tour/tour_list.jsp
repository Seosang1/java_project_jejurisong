...
%> 
			...
			<article>
				<h3>여행지 리스트 > 여행지 리스트</h3>
				
				<!-- content -->
				<div id="content">
				
					<!-- *** Search Layout *** -->
					<form name="frmSearch" method="get" action="tour_list.toura">
					...
					<div class="table_top">
						...
						<span class="btn_event" style="display:block;position:absolute;top:0px;right:0px;">
							<a href="tour_form.toura" class="btn_default blue">등록</a>
					...
					<div class="tbl_wrap_list">
						...
						if (list != null && list.size() > 0) {
							...
							for (int i = 0 ; i < list.size() ; i++) {
						      	TourInfo tourInfo = list.get(i); 

						      	String dis = "";
						      	if(tourInfo.getTour_del_yn().equals("y")) {
						      		dis = "disabled";
						      	}
						%> 
							<tr onmouseover="this.className='on'" onmouseout="this.className=''" onclick="location.href='#';">
								<td><input type="checkbox" name="chk_del" value="<%=tourInfo.getTour_code() %>" <%=dis %>/></td>
								<td><%=common.getArea().get(tourInfo.getTour_area()) %></td>
								<td><%=common.getTourType().get(tourInfo.getTour_type()) %></td>
								<td><strong><a href="tour_view.toura?tour_code=<%=tourInfo.getTour_code() %>"><%=tourInfo.getTour_name() %></a></strong></td> 
								<td><%=tourInfo.getTour_tag() %></td>
								<td><%=tourInfo.getTour_score() %></td>
								<td><%=tourInfo.getTour_review_cnt() %></td>  
								<td><%=common.getOpenYn().get(tourInfo.getTour_open_yn()) %></td>
								<td>
								<% if(tourInfo.getTour_del_yn().equals("n")) { %>
								<input type="button" value="삭제" onclick="onProcDel('<%=tourInfo.getTour_code() %>')" />
								<% } else { %>
									삭제완료
								<% } %>
								</td>
				...
				<% if (list != null && list.size() > 0) {
						...
						if (pageInfo.getCpage() > 1) { 
							firstUrl = "tour_list.toura" + args + "&cpage=1";
							prevUrl  = "tour_list.toura?cpage=" + (pageInfo.getCpage() - 1);
						} // 첫 페이지와 이전 페이지 링크
					%>
					...
					<%  
						for (int i = 1, k = pageInfo.getSpage() ; i <= pageInfo.getBsize() && k <= pageInfo.getEpage() ; i++, k++) {
							if (pageInfo.getCpage() == k) { %> 
								<a href='#' class='on'><%=k %></a>
					<%		} else { %>
								<a href="tour_list.toura<%=args + "&cpage=" + k %>"><%=k %></a> 
					<%		}
						}
						if (pageInfo.getCpage() != pageInfo.getPcnt()) {	// 현재 페이지번호가 마지막 페이지 번호이면
							lastUrl = "tour_list.toura" + args + (pageInfo.getCpage() + 1);
							nextUrl = "tour_list.toura" + args + "&cpage=" + pageInfo.getPcnt(); 
						} 
					%> 
						</span>
