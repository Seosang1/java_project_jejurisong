...
%> 
				...
				function onProcDel(code) {
					...
					if (arrCheck.length == 0) {
						...
						if (confirm("정말 삭제하시겠습니까?")) {
					        ...
					        frm.setAttribute("action", "food_reply_del.fooda"); //요청 보낼 주소
			...
			<article>
				<h3>음식점 리스트 > 후기 리스트</h3>
				
				<!-- content -->
				<div id="content">
				
					<!-- *** Search Layout *** -->
					<form name="frmSearch" method="get" action="food_reply_list.fooda">
					...
					<div class="tbl_wrap_list">
						...
						%>
							<tr onmouseover="this.className='on'" onmouseout="this.className=''" onclick="location.href='#';">
								...
								<td><%=common.getArea().get(replyInfo.getFood_area()) %></td> 
								<td><%=common.getFoodType().get(replyInfo.getFood_type()) %></td> 
								<td><%=replyInfo.getFood_name() %></td> 
				...
				<% if (list != null && list.size() > 0) {
						...
						if (pageInfo.getCpage() > 1) { 
							firstUrl = "food_reply_list.fooda" + args + "&cpage=1";
							prevUrl  = "food_reply_list.fooda?cpage=" + (pageInfo.getCpage() - 1);
						} // 첫 페이지와 이전 페이지 링크
					%>
					...
					<%  
						for (int i = 1, k = pageInfo.getSpage() ; i <= pageInfo.getBsize() && k <= pageInfo.getEpage() ; i++, k++) {
							if (pageInfo.getCpage() == k) { %> 
								<a href='#' class='on'><%=k %></a>
					<%		} else { %>
								<a href="food_reply_list.fooda<%=args + "&cpage=" + k %>"><%=k %></a> 
					<%		}
						}
						if (pageInfo.getCpage() != pageInfo.getPcnt()) {	// 현재 페이지번호가 마지막 페이지 번호이면
							lastUrl = "food_reply_list.fooda" + args + (pageInfo.getCpage() + 1);
							nextUrl = "food_reply_list.fooda" + args + "&cpage=" + pageInfo.getPcnt(); 
						} 
					%> 
						</span>
