...
public class FoodReplyDao {
	private static FoodReplyDao ReplyDao;
	private Connection conn;

	private FoodReplyDao() {}

	public static FoodReplyDao getInstance() {
		if (ReplyDao == null)
			ReplyDao = new FoodReplyDao();
		return ReplyDao;
	}
	...
	public ArrayList<ReplyInfo> getList(String where, String orderBy, int cpage, int psize) {
		...
		try {
			...
			String sql = " select food.food_name, food.food_type, reply.*  from t_reply_list reply left join t_food_info food on reply.reply_code = food.food_code " + where + orderBy;
	...
	public ReplyInfo getSelectOne(String where) {
		...
		try {
			String sql = " select food.food_name, reply.*  from t_reply_list reply left join t_food_info food on reply.reply_code = food.food_code " + where ;
	...
	public int insert(ReplyInfo ReplyInfo) {
		...
		try {  
			String sql = "call sp_food_reply_insert(?, ?, ?, ?);";
		...
		try {
			...
			for (int i = 0; i < arrRvCode.length; i++) {
				...
				if (rs.next()) {
					...
					sql = "call sp_food_reply_delete(" + reply_idx + ", '" + reply_code + "', '"+ del_id + "')";
					System.out.println("* call sp_food_reply_delete : " + sql);
