...
public class TourReplyDao {
	private static TourReplyDao ReplyDao;
	private Connection conn;

	private TourReplyDao() {}

	public static TourReplyDao getInstance() {
		if (ReplyDao == null)
			ReplyDao = new TourReplyDao();
		return ReplyDao;
	}
	...
	public ArrayList<ReplyInfo> getList(String where, String orderBy, int cpage, int psize) {
		...
		try {
			...
			String sql = " select tour.tour_name, tour.tour_type, reply.*  from t_reply_list reply left join t_tour_info tour on reply.reply_code = tour.tour_code " + where + orderBy;
