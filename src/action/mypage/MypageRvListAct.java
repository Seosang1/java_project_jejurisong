...
import svc.hotel.*;
import svc.mypage.MypageReplySvc;
import vo.*;

public class MypageRvListAct implements Action {
//  
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, Exception {
		...
		/* 예약 완료 리스트 */
		where = " where mem_id = '" + loginMember.getMi_id() +"' and rv_state = 'a' ";
		ArrayList<ReserveInfo> rv_list_a = listAdminSvc.getList(where, "order by rv.rv_idx desc", 0, -1);
		
		/* 예약 취소 리스트 */	
		where = " where mem_id = '" + loginMember.getMi_id() +"' and rv_state = 'b' ";
		ArrayList<ReserveInfo> rv_list_b = listAdminSvc.getList(where, "order by rv.rv_idx desc", 0, -1);
		...
		request.setAttribute("rv_list_a", rv_list_a);  					// 예약 리스트
		request.setAttribute("rv_list_b", rv_list_b);					// 예약 취소 리스트 
		request.setAttribute("nowPage", "mypage_rv.mypage");			// 현재 페이지 
		 
		ActionForward forward = new ActionForward();
		forward.setPath("/mypage/mypage_rv.jsp"); 
