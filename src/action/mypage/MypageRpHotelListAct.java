...
public class MypageRpHotelListAct implements Action {
//  
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, Exception {
		...
		/* 숙박 후기 */ 
		where = " where reply_type='A' and mi_id = '" + loginMember.getMi_id() +"' and reply_del_yn = 'n' ";
		ArrayList<ReplyInfo> hotel_rp_list = mypageReplySvc.getHotelReplyList(where, "", 0, -1);	
		...
		request.setAttribute("hotel_rp_list", hotel_rp_list);			// 호텔 후기 리스트
		request.setAttribute("nowPage", "mypage_rp_hotel.mypage");		// 현재 페이지
		 
		ActionForward forward = new ActionForward();
		forward.setPath("/mypage/mypage_rp_hotel.jsp"); 
