...
public class MypageRpTourListAct implements Action {
//  
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, Exception {
		...
		/* 여행지 후기 */ 
		where = " where reply_type='T' and mi_id = '" + loginMember.getMi_id() +"' and reply_del_yn = 'n' ";
		ArrayList<ReplyInfo> rp_list = mypageReplySvc.getTourReplyList(where, "", 0, -1);	
		...
		request.setAttribute("rp_list", rp_list);						// 여행지 후기 리스트
		request.setAttribute("nowPage", "mypage_rp_tour.mypage");		// 현재 페이지
		 
		ActionForward forward = new ActionForward();
		forward.setPath("/mypage/mypage_rp_tour.jsp"); 
