...
import svc.member.MemberSvc;
import svc.tour.*;
import svc.user.*;
import vo.*;

public class MemberDelAdminAct implements action.Action{
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (request.getParameter("arrCheck") == null || request.getParameter("arrCheck").equals("")) {
			...
			out.println("<script>");
			out.println("alert('아이디 삭제에 실패하였습니다.');");
		...
		HashMap<String, String> ret = memberSvc.delete(whereInDel, tourRegId);
	    ...
	    forward.setPath("/jejurisong/admin/member_list.membera");
