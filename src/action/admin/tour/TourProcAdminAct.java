...
public class TourProcAdminAct implements action.Action {
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String uploadPath = imgUrl + "jejurisong/WebContent/common/images/TourImg";
...
//		System.out.println("-----------------------------");
		...
		String tour_code = tourProcAdminSvc.getCode(); 
		tourInfo.setTour_code(tour_code); 				// 여행지코드 가져옴		
		int result = tourProcAdminSvc.insert(tourInfo); 	// 여행지정보 저장 결과여부
		if (result == 0) {
			response.setContentType("text/html; charset=utf-8");
			PrintWriter out = response.getWriter();
			out.println("<script>");
			out.println("alert('여행지정보 등록에 실패하였습니다.');");
